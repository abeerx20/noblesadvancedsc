...
app.use("/client", express.static(clientDirectory, { extensions: ["html"], maxAge: env.NODE_ENV === "production" ? "1h" : 0 }));
...
app.get(/^\/((?!api\/)[^?#]+)\.(js|css|html)$/i, (req, res, next) => {
  ...
  if (candidate.startsWith(path.resolve(currentDirectory, "../..")) && existsSync(candidate) && statSync(candidate).isFile()) {
