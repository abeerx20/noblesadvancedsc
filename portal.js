...
<div>${adminTitle}</div>
  ...
  `;
    ...
    const setPermissionDay = () => {
      if (!requestDateField || !requestDayField) return;

      const dayNames = ["الأحد", "الإثنين", "الثلاثاء", "الأربعاء", "الخميس", "الجمعة", "السبت"];
      const rawValue = requestDateField.value || "";
      ...
      if (!rawValue) {
        ...
        requestDateField.value = `${year}-${month}-${day}`;
      }

      const normalizedValue = (requestDateField.value || "").replace(/\//g, "-");
    ...
    if (requestDateField) {
      requestDateField.addEventListener("input", setPermissionDay);
      requestDateField.addEventListener("change", setPermissionDay);
    }
    if (requestDayField) {
      requestDayField.addEventListener("input", () => {
        if (!requestDayField.value.trim()) {
          setPermissionDay();
        }
      });
    }
