...
function page(title, description, body) {
  ...
  content.innerHTML = `<section class="content-card page-card">${breadcrumb}<div class="page-heading"><div><h1>${title}</h1></div>${backButton}</div><div id="pageNotice" class="notice" role="alert" aria-live="polite"></div>${body}</section>`;
...
async function renderRequestPage(key, overrides = {}, viewKey = key) {
  ...
  const attachmentField = config.attachmentType === "url"
    ? `<div class="field span-2"><label for="requestAttachment">رابط المرفق (اختياري)</label><input id="requestAttachment" type="url" placeholder="https://..."><span class="field-hint">أضيفي رابط المستند أو المرفق إن وُجد.</span></div>`
    : config.attachment && has("upload_files")
      ? `<div class="field span-2"><label for="requestAttachment">المرفق (اختياري)</label><input id="requestAttachment" type="file" accept=".pdf,.jpg,.jpeg,.png"><span class="field-hint">PDF أو JPG أو PNG، بحد أقصى 5 ميغابايت.</span></div>`
      : "";
